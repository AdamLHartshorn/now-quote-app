...
export default function MileageLookup({ pickup, delivery, onPickupChange, onDeliveryChange, onMileage }: { pickup: string; delivery: string; onPickupChange: (value: string) => void; onDeliveryChange: (value: string) => void; onMileage: (miles: number) => void }) {
  ...
  return <section className="panel space-y-4"><div><p className="panel-title !mb-1">Route mileage</p><p className="text-xs leading-5 text-slate-500">Enter a full address or a recognizable location. Calculated mileage remains editable below.</p></div><label><span className="field-label">Pickup</span><input className="control" value={pickup} onChange={(event) => onPickupChange(event.target.value)} placeholder="Pickup address or location" /></label><label><span className="field-label">Delivery</span><input className="control" value={delivery} onChange={(event) => onDeliveryChange(event.target.value)} placeholder="Delivery address or location" /></label><button type="button" className="secondary-button !border-[#8fc2ca] !text-[#00798d]" disabled={loading || !pickup.trim() || !delivery.trim()} onClick={calculate}>{loading ? "CALCULATING DRIVING ROUTE…" : "CALCULATE DRIVING MILES"}</button>{error && <p className="text-xs font-bold text-red-600">{error}</p>}{result && <div className="rounded-xl bg-[#edf7f8] p-3 text-xs leading-5 text-[#38515e]"><p className="font-extrabold text-[#00798d]">{result.miles} miles · about {result.minutes} minutes</p><p className="mt-1 text-[10px] text-slate-500">Pickup match: {result.pickupMatch}</p><p className="text-[10px] text-slate-500">Delivery match: {result.deliveryMatch}</p></div>}</section>;
}
