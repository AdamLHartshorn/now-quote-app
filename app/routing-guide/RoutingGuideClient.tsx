...
function CreateRoute({ onCancel, onCreated }: { onCancel: () => void; onCreated: (route: SavedRoute) => void }) {
  ...
  return <>
    ...
    <form className="form-stack" onSubmit={submit}>
      ...
      <section className="panel space-y-4"><div><p className="panel-title !mb-1">Starting location</p><p className="text-xs text-slate-500">The donut shop is always locked as the route start.</p></div><label><span className="field-label">Donut shop name</span><input required className="control" value={startName} onChange={(event) => setStartName(event.target.value)} placeholder="Donut shop name" /></label><label><span className="field-label">Full address</span><input required className="control" value={startAddress} onChange={(event) => setStartAddress(event.target.value)} placeholder="Street, city, state ZIP" /></label></section>
      <section className="panel space-y-5">
        <div className="flex items-start justify-between"><div><p className="panel-title !mb-1">Prospects</p><p className="text-xs text-slate-500">{prospects.length} of 20</p></div>{prospects.length < 20 && <button type="button" onClick={() => setProspects((current) => [...current, blankProspect()])} className="text-xs font-extrabold text-[#007f94]">+ ADD</button>}</div>
        {prospects.map((prospect, index) => <div key={index} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex justify-between"><span className="text-xs font-extrabold text-[#102d3d]">PROSPECT {index + 1}</span>{prospects.length > 1 && <button type="button" onClick={() => setProspects((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="text-xs font-bold text-red-500">Remove</button>}</div><div className="space-y-3"><input required className="control" value={prospect.businessName} onChange={(event) => updateProspect(index, "businessName", event.target.value)} placeholder="Business name" /><input required className="control" value={prospect.address} onChange={(event) => updateProspect(index, "address", event.target.value)} placeholder="Full address" /></div></div>)}
      </section>
      {error && <p className="rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</p>}
...
function StopCard({ stop, index, onStatus, onMove, first, last }: { stop: RouteStop; index: number; onStatus: (status: StopStatus) => void; onMove: (direction: -1 | 1) => void; first: boolean; last: boolean }) {
  ...
  return <article className={`rounded-2xl border p-4 shadow-sm ${tone}`}><div className="flex gap-4"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#15394b] text-sm font-extrabold text-white">{index + 1}</div><div className="min-w-0 flex-1"><h3 className={`font-extrabold text-[#102d3d] ${stop.status === "completed" ? "line-through opacity-60" : ""}`}>{stop.businessName}</h3><p className="mt-1 text-sm leading-5 text-slate-500">{stop.address}</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => onStatus(stop.status === "completed" ? "pending" : "completed")} className="rounded-lg bg-emerald-100 px-3 py-2 text-[10px] font-extrabold text-emerald-700">{stop.status === "completed" ? "UNDO" : "COMPLETE"}</button><button type="button" onClick={() => onStatus(stop.status === "skipped" ? "pending" : "skipped")} className="rounded-lg bg-amber-100 px-3 py-2 text-[10px] font-extrabold text-amber-700">{stop.status === "skipped" ? "UNDO" : "SKIP"}</button><button disabled={first} type="button" onClick={() => onMove(-1)} className="rounded-lg bg-slate-100 px-3 py-2 text-[10px] font-extrabold text-slate-600 disabled:opacity-30">↑</button><button disabled={last} type="button" onClick={() => onMove(1)} className="rounded-lg bg-slate-100 px-3 py-2 text-[10px] font-extrabold text-slate-600 disabled:opacity-30">↓</button></div></div></div></article>;
}
