  ...
  return <>
    ...
    <form className="form-stack" onSubmit={submit}>
      ...
      <section className="panel space-y-5">
        ...
        {prospects.map((prospect, index) => <div key={index} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex justify-between"><span className="text-xs font-extrabold text-[#102d3d]">PROSPECT {index + 1}</span>{prospects.length > 1 && <button type="button" onClick={() => setProspects((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="text-xs font-bold text-red-500">Remove</button>}</div><div className="space-y-3"><input required className="control" value={prospect.businessName} onChange={(event) => updateProspect(index, "businessName", event.target.value)} placeholder="Business name" /><input required className="control" value={prospect.address} onChange={(event) => updateProspect(index, "address", event.target.value)} placeholder="Address, 30th St, Franklin, etc." /></div></div>)}
